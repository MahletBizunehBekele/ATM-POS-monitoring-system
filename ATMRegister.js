...
import Validation from '../ATM&POS/ATMRegValidation';


function ATMRegister(){
    ...
    const handleSubmit = async (event) => {
        ...
        if (Object.keys(validationErrors).length === 0) {  
            console.log("ATMRegister")          
            try {
                const response = await axios.post('/atmreg',  {
                    atm_name: values.HostName,
    ...
    return(
        <div className='d-flex justify-content-center align-items-center bg-light h-220' style ={{marginTop: "100px"}}>
            <div style={{maxWidth: '400px', color: "white", marginRight:"150px", marginTop:"10px"}} className='bg-light rounded w-50 p-3'>
                <h2 className='text-dark mb-4'>ATM Registration</h2>
                <form action="">
                ...
                <div className="mb-3 text-secondary">
                    <label htmlFor="minimum_amount"><strong>Minumum Amount</strong></label>
                        <input
                            type="number"
                            id="minimum_amount"
                            ...
                            className='form-control rounded-0'
                        />
                    {errors.minimum_amount && <span className='text-danger'>{errors.minimum_amount}</span>}
                </div>
                <div className="mb-3 text-secondary">
                    ...
                    <div className="mb-3 text-secondary">
                        ...
                        <textarea
                            id="description"
                            name="description"
                            placeholder="Enter ATM description"
                            onChange={handleInput}
                            className="form-control rounded-0"
                            style={{ height: '80px' }}
                        />
